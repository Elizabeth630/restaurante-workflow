<?php
// ==================== ARCHIVO: ctrl.factura_directa.inc.php ====================

if (isset($_GET["Siguiente"])) {
    if (!isset($_GET["pedido_id"]) || !isset($_GET["total"]) || !isset($_GET["metodo_pago"])) {
        die("Error: Parámetros incompletos para facturación directa");
    }
    ...
    if (mysqli_num_rows($check_factura) > 0) {
        ...
        $sql = "INSERT INTO facturas (pedido_id, total, metodo_pago, fecha) VALUES (?, ?, ?, NOW())";
        $stmt = mysqli_prepare($con, $sql);
        mysqli_stmt_bind_param($stmt, "ids", $pedido_id, $total, $metodo_pago);
    }
    
    if (!mysqli_stmt_execute($stmt)) {
        die("Error al guardar factura directa: " . mysqli_error($con));
    }
    ...
    error_log("Factura directa completada para pedido $pedido_id por cocinero");
}
?>